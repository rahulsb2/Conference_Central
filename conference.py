...
MEMCACHE_ANNOUNCEMENTS_KEY="announcement"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - Conference objects - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(message_types.VoidMessage,ConferenceForms,path='filterPlayground',http_method='POST',name='filterPlayground')
    def filterPlayground(self,request):
        conferences=Conference.query()        
        field="city"
        operator="="
        value="London"
        q=ndb.query.FilterNode(field,operator,value)
        conferences=conferences.filter(q)
        conferences=conferences.filter(Conference.month==6)
        return ConferenceForms(items=[self._copyConferenceToForm(conff,"") for conff in conferences])
# - - - Registration - - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(CONF_GET_REQUEST,ConferenceForms,path='getConference',http_method="POST",name='getConference')
    def getConference(self,request):
        wsck = request.websafeConferenceKey
        conferences=ndb.Key(urlsafe=wsck).get()
        if not conferences:
            raise endpoints.NotFoundException(
                'No conference found with key: %s' % wsck)
        return ConferenceForms(items=[self._copyConferenceToForm(conferences,"")] ) 
    ...
    def getConferencesToAttend(self, request):
        ...
        k=[]
        for key in keys:
            k.append(ndb.Key(urlsafe=key))
        conferences=ndb.get_multi(k)
